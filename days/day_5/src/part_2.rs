...
/// Swap vector numbers until they are agreeing with the rules.
fn fix_update(update: &mut Vec<u8>, rules: &Vec<Rule>) {
    let mut idx = 0;

    'outer: while idx < update.len() {
        let number = &update[idx];
